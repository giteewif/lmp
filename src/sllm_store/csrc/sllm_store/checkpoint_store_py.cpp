...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  py::class_<MemCopyChunk>(m, "MemCopyChunk")
      ...
      .def_readwrite("task_id", &MemCopyChunk::task_id_)
      .def_readwrite("priority", &MemCopyChunk::priority_)
  ...
  py::class_<CheckpointStore>(m, "CheckpointStore")
      ...
      .def(
          "submit_high_priority_tasks",
          [](CheckpointStore& cs, const std::string& model_path,
             const std::string& replica_uuid,
             const std::vector<uint64_t>& task_ids) {
            std::vector<uint64_t> pending_task_ids;
            int code = cs.SubmitHighPriorityTasks(model_path, replica_uuid,
                                                  task_ids, &pending_task_ids);
            return py::make_tuple(code, pending_task_ids);
          },
          py::arg("model_path"), py::arg("replica_uuid"), py::arg("task_ids"),
          "Submit high priority copy tasks by task id.")
      .def("set_reorder_bitmap", &CheckpointStore::SetReorderBitmap,
           py::arg("model_path"), py::arg("replica_uuid"),
           py::arg("task_ids"),
           "Set low-priority reorder bitmap by task ids.")
      .def(
          "wait_copy_tasks",
          [](CheckpointStore& cs, const std::string& model_path,
             const std::string& replica_uuid,
            ...
            std::vector<uint64_t> pending_task_ids;
            int code = cs.WaitCopyTasks(model_path, replica_uuid, task_ids,
                                        timeout_ms, &pending_task_ids);
            return py::make_tuple(code, pending_task_ids);
          },
          py::arg("model_path"), py::arg("replica_uuid"), py::arg("task_ids"),
          py::arg("timeout_ms") = 30000,
          "Wait for a set of copy tasks to finish.")
