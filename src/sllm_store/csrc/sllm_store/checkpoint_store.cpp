...
CheckpointStore::CheckpointStore(const std::string& storage_path,
  ...
  if (use_shared_memory_) {
    memory_pool_shared_ =
        std::make_shared<PinnedMemoryPoolShared>(memory_pool_size_, chunk_size_,
                                                 shm_name_prefix_);
    LOG(INFO) << "Shared memory pool created with " << memory_pool_size_ / GB
              << "GB, prefix: " << shm_name_prefix_;
  } else {
    memory_pool_ =
...
int CheckpointStore::LoadModelFromDisk(const std::string& model_path) {
  ...
  int remaining_size;
  if (use_shared_memory_) {
    remaining_size = model->AllocatePinnedMemory(memory_pool_shared_);
  } else {
    remaining_size = model->AllocatePinnedMemory(memory_pool_);
  }
  if (remaining_size < 0) {
