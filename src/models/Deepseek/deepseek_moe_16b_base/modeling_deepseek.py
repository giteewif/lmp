...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids, unsqueeze_dim=1):
    ...
    """
    # print(f"cos shape {cos.shape} sin shape {sin.shape} position_ids {position_ids}")
...
class DeepseekModel(DeepseekPreTrainedModel):
    ...
    def forward(
        ...
        print(f"before sdpa attention attention_mask {attention_mask} position_ids {position_ids}")
        ...
        print(f"after sdpa attention attention_mask {attention_mask} position_ids {position_ids}")
    ...
    def prepare_inputs_for_generation(
        ...
        if past_key_values is not None:
            if isinstance(past_key_values, Cache):
                ...
                # Handle different versions of DynamicCache
                if hasattr(past_key_values, 'seen_tokens'):
                    past_length = past_key_values.seen_tokens
                else:
                    past_length = cache_length
                # Handle different versions of DynamicCache for max_length
