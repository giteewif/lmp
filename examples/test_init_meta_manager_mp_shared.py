...
if __name__ == '__main__':
    ...
    imm = InitMetaManagerMPShared()

    time_hook("start_workers")
    imm.start()
    time_hook_end("start_workers")

    for i in range(3):
        time_hook("submit_all")
        # 一次性提交所有 layer 的初始化任务
        # 注意：这个版本在主进程中初始化所有 layer，然后共享给工作进程
        imm.submit_all(init_layer_func, config=config)
        time_hook_end("submit_all")
        
        
        time_hook("wait_all")
        imm.wait_all()
        time_hook_end("wait_all")
    

    # 停止所有工作进程
    time_hook("stop_workers")
    imm.stop()
    time_hook_end("stop_workers")
