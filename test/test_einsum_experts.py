...
model_path = "/mnt/zhengcf3/models/opt-6.7b"
print("正在加载模型...")
...
print(f"init opt model cost {time.time() - time_start} s")
...
print(f"init model cost {time.time() - time_start} s")
...
for i in range(2):
    input_token = torch.randn(64*64, h2, dtype=dtype, device="cpu")
    inputs_tokens[i] = input_token
for i in range(times):
    time_start_once = time.time()
    _, time_exec= einsum_experts(layer2, inputs_tokens)
    time_end_once = time.time() - time_start_once
    time_once_lists.append(time_exec)
